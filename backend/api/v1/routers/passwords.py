...
async def get_user_passwords(
        data: user_schema.GetUserBySecret,
...
@password_router.post("", response_model=password_schema.PasswordResponse, status_code=status.HTTP_201_CREATED)
async def create_password(
    ...
    db_password = await password_crud.create_password(db=db, password=password, user_id=user.id)
...
@password_router.patch("/{password_id}", response_model=password_schema.PasswordResponse)
...
async def delete_password(
    ...
    deleted = await password_crud.delete_password(db, password_id=password_id, user_id=user.id)
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Password not found"
